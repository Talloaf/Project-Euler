...
def product_vertical(array, compare)
	for y in 0...-4
		for x in 0...-1
			value = array[y][x]*array[y+1][x]*array[y+2][x]*array[y+3][x]
...
def product_horizontal(array, compare)
	for y in 0...-1
		for x in 0...-4
			value = array[y][x]*array[y][x+1]*array[y][x+2]*array[y][x+3]
			compare = value if value > compare
		end
	end
	return compare
...
def product_diagonal_down (array, compare)
	for y in 0...-4
		for x in 0...-4
			value = array[y][x]*array[y+1][x+1]*array[y+2][x+2]*array[y+3][x+3]
			compare = value if value > compare
		end
	end
	return compare
...
def product_diagonal_up (array, compare)
	for y in 3...20
		for x in 0...17
			value = array[y][x]*array[y-1][x+1]*array[y-2][x+2]*array[y-3][x+3]
			compare = value if value > compare
...
highest_product = product_vertical(num_array, highest_product)
highest_product = product_horizontal(num_array, highest_product)
highest_product = product_diagonal_down(num_array, highest_product)
highest_product = product_diagonal_up(num_array, highest_product)
